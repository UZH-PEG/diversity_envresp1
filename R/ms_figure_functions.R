...
fig_state_vs_o2diff_sidebyside_alternative_repmeth <- function(ss_result){
  ...
  direction.facet <- ggplot(data.frame(x = 1, y = 1,direction = c("High initial CB density","Low initial CB density"))) +
    geom_text(aes(x, y, label = direction), size=4.5) +
    facet_wrap(~direction)+
    theme_bw()+
    theme(strip.background = element_blank(),
