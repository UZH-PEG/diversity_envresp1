...
```{r div-temp-eventdef1, eval = FALSE, echo = FALSE,  include=TRUE, fig.cap='(ref:div-temp-eventdef1)', fig.height=8, fig.width=8, fig.align="center"}
all_stab <- readRDS(here("data",
                          microxanox_version_path,
                          "temporal_method",
                          "event_definition_1",
...
```{r ss-temp-meth3, eval = FALSE, echo = FALSE,  include=TRUE, fig.cap='(ref:ss-temp-meth3)', fig.height=10, fig.width=8, fig.align="center"}
...
```{r ss-temp-meth4, eval = FALSE, echo = FALSE,  include=TRUE, fig.cap='(ref:ss-temp-meth4)', fig.height=10, fig.width=8, fig.align="center"}
...
To test if the diversity effects on resilience were driven entirely by the most tolerant strains, we made additional simulations where functional groups contained only the strain with greatest tolerance and lowest maximum growth rate. That is, for each of the 20 levels of trait variation, we ran simulations where each functional group had only one strain, and its trait values corresponded with those of the most tolerant strain for the given level of trait variation. We also manipulated in which of the functional groups tolerance increased. For example, in a simulation to be compared with variation in only the SB group (and no variation in the CB or PB groups), the SB group contained the strain with highest tolerance, whereas CB and PB each contained the strain with mean tolerance. We then compared the tipping point positions resulting from this tolerance manipulation with the tipping point positions resulting from the diversity manipulation presented in the main text.
...
For most of the explored combinations and amounts of functional diversity/tolerance the tipping point positions did not differ between the diversity manipulation and the tolerance manipulation (Figure \@ref(fig:max-tol1)). However, the tipping point positions did differ between the diversity manipulation and the tolerance manipulation when diversity/tolerance in the SB group was high (i.e. in the combinations SB only, CB-SB, and CB-SB-PB). We show below that 


(ref:max-tol1) Effects of strain variation on position of tipping points compared between simulations with all (nine) strains present and simulations where only the most tolerant strain is present. In the SB and CB-SB treatments there is missing data in the case with only the most tolerant strain when there were no alternate stable states and therefore no tipping points.
...
all_stab <- all_stab %>%
  filter(!(var_treat == "SB" & stand_var > 0.88 & num_strains == -1),
         !(var_treat == "CB-SB" & stand_var > 0.84 & num_strains == -1))
...
When the SB contained only one strain with very high tolerance and low maximum growth rate, there were no alternate stable states, i.e. the state of the system was independent of the direction of change in oxygen diffusivity (Figure \@ref(fig:max-tol3)). Furthermore, at low oxygen diffusivity the system varied around a stable state with all three functional groups coexisting –this never occurred when there were multiple strains varying in tolerance (Figure \@ref(fig:max-tol2)), or when there was only one strain with mean tolerance.

Apparently, absence of SB strains with high growth rate precludes the formation of a stable state that does not include cyanobacteria. It seems that the most tolerant SB strain does not have a high enough growth rate to itself exclude the cyanobacteria. Cyanobacteria are therefore always present, regardless of the oxygen diffusivity.
...
max_tol_one <- filter(maxtolonly_strains,
       ...
       abs(SB_var_gmax_s - 0.09473684) < 0.001,
       PB_var_gmax_s == 0)
fig_state_vs_o2diff_sidebyside_alternative_nofliparrows(max_tol_one)
```

### What causes the variation in response to oxygen diffusivity (SB)?
...
## High tolerance vs. high diversity in CB and SB
...
(ref:max-tol4)  Stables states found when there is high trait variation among nine strains of CB and SB. All other aspects are the same as in Figure \@ref(fig:repl-meth2). 
...
filter(nine_strains,
       ...
       PB_var_gmax_s == 0) %>%
fig_state_vs_o2diff_sidebyside_alternative_leftfliponly()
```
...
### What causes the variation in response to oxygen diffusivity (CB-SB)?

In the ecosystem shown in Figure \@ref(fig:max-tol5), where CB and SB each contained only one strain with very high tolerance, there was large variation in state variables along the oxygen diffusivity gradient at values less than about -6.5. To examine what might be causing this, we simulated the ecosystem at a constant oxygen diffusivity of $log_{10}$ oxygen diffusivity of -7 (Figure \@ref(fig:CBSB-maxtol-dyn1)). We see that the dynamics are qualitatively different from other cases in which there is a stable state reached in such a simulation (Figure \@ref(fig:sb-div-effect4)). It appears that the SB strain with high tolerance is able to initiate a transition to an anoxic state, but is not able to create a stable anoxic state. 
...
When in each of the three functional groups only the most tolerant strain is present, the resilience of the oxic state is greater than when there is variation in strain tolerance within groups. Looked at from another perspective, comparing CB-SB and CB-SB-PB both with only the most tolerant strains, presence of a high tolerance PB strain (in CB-SB-PB) allows the formation of an anoxic cyanobacteria-free stable state (whereas there was no such state in CB-SB). We interpret this as presence of a high tolerance PB strain stabilising the anoxic state, perhaps by increasing the availability of oxidised sulfur which is a resource for SB. Nevertheless, absence of SB strains with high growth rate seems to shift the oxic-anoxic tipping point to lower oxygen diffusivity. 
