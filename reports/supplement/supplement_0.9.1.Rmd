...
The rate of change in population densities is given by the per capita growth rate - per capita death rate, multipled by population size ([Bush et al 2017](https://www.nature.com/articles/s41467-017-00912-x)). Growth rate is the growth rate in absence of any inhibition multiplied by the amount of inhibition. 
...
Growth of cyanobacteria depended on one substrate (phosphorus), whereas growth of sulfate-reducing bacteria (SB) and phototrophic sulfur bacteria (PB) depended on two substrates (phosphorus and oxidised sulfur for SB, and phosphorus and sulfide for PB).
...
plot_here <- all_stab %>%
  ...
  ## remove data where there are non-standard stable states
## this subset is identified by inspection of stable state figures
  filter(!(num_strains == 9 & var_treat == "CB" & stand_var > 1.00),
         !(num_strains == 9 & var_treat == "SB" & stand_var > 1.00),
         !(num_strains == 9 & var_treat == "PB" & stand_var > 0.70),
...
plot_here <- all_stab %>%
  ...
  filter(!(num_strains == 9 & var_treat == "CB" & stand_var > 1.00),
         ...
         !(num_strains == 9 & var_treat == "CB-SB" & stand_var > 1.00),
         !(num_strains == 9 & var_treat == "CB-PB" & stand_var > 0.65),
         !(num_strains == 9 & var_treat == "SB-PB" & stand_var > 1.00),
         !(num_strains == 9 & var_treat == "CB-SB-PB" & stand_var > 1.00))
p1 <- fig_div_vs_o2diff_1strain_7row(plot_here,
                                     which_strain = num_strains,
                                     figure_title = paste0(wait_time,
...
The results below show the stable states as a function of oxygen diffusivity for a system with an intermediate level of diversity in all functional groups, with either 1'000'000 hours at each oxygen diffusivity level (Figure \@ref(fig:ss-temp-meth3)), or 2'000'000 hours (Figure \@ref(fig:ss-temp-meth4)). This shows that 1'000'000 hours duration of the simulations are sufficient to reach stable state, except in regions of strain replacement, including at some state transitions. E.g. in the region of strain replacement in the sulfate-reducing bacteria recovery trajectory there is difference in strain abundances at 1'000'000 and 2'000'000 hours. However, the position of the tipping points (which were calculated from the total SB abundance) and the substrate concentrations did not differ between 1'000'000 and 2'000'000 hours.
...
fig_div_vs_o2diff_multistrain(plot_here111, which_strains,
...
fig_state_vs_o2diff_sidebyside_alternative(only_CB_variable[20,])
...
When there was medium to high variation in only the phototrophic sulfur bacteria (PB), the strain dynamics deviated from the standard pattern: at low oxygen diffusivity, all three functional groups coexisted. Specifically, on the trajectory of decreasing oxygen diffusivity, cyanobacteria reappeared at low oxygen diffusivity after their collapse at medium oxygen diffusivity (medium variation in PB, Figure xy) or never collapsed (high variation in PB, Figure xy). On the trajectory of increasing oxygen diffusivity, cyanobacteria coexisted with the two groups of sulfur bacteria at low oxygen diffusivity, collapsed at medium levels of oxygen diffusivity, and then increased to sole dominance at the tipping point to the oxic state. 

The same pattern occurred for medium to high simultaneous variation in phototrophic sulfur bacteria and cyanobacteria, albeit only on the trajectory of increasing oxygen diffusivity (Figure xy). On the trajectory of decreasing oxygen diffusivity, cyanobacteria dominated over the entire gradient of oxygen diffusivity, such that effects of PB diversity did not play out on this trajectory. 

The reason for coexistence of all three groups at low oxygen diffusivity was that the PB strain that dominated at low oxygen diffusivity (the least tolerant strain) lowered the sulfide concentrations to such low levels that cyanobacteria were able to grow. The higher the trait variation in PB, the higher the intrinsic growth rate of the least tolerant PB strain, and the higher its consumption of sulfide. Consequently, moderate variation in PB lowered the sulfide concentration such that the tipping point from anoxic to oxic occurred earlier (as discussed in the previous section), and medium to high variation in PB lowered the sulfide concentration even further such that cyanobacteria were able to grow and coexist with the two groups of sulfur bacteria at low oxygen diffusivity. 

At intermediate levels of oxygen diffusivities, the cyanobacteria were usually excluded (Figure xy). We speculate that this might be associated with the increase in sulfide prior to the collapse of the phototrophic sulfur bacteria. This increase in sulfide occurred simultaneously with turnover to PB strains with lower intrinsic growth rate and with a decline in PB density; these two processes likely resulted in less consumption of sulfide. 



Owen, please add here 3 figures that match the text above: 
...
(1) medium variation in only PB showing coexistence of all groups at low ox diff, 
...
nine_strain_one <- filter(nine_strains,
       CB_var_gmax_s == 0,
       SB_var_gmax_s == 0,
       abs(PB_var_gmax_s - 0.064819946) < 0.001)

parameter <- nine_strain_one$parameter[[1]]
parameter$strain_parameter <- nine_strain_one$pars[[1]]
...
(ref:PB-dyn1) Temporal dynamics of an ecosystem with a , the same one as in Figure \@ref(fig:max-tol3) simulated with constant $log_{10}$ oxygen diffusivity of -7.

```{r PB-dyn1, echo = FALSE,  include=TRUE, fig.cap='(ref:PB-dyn1)', fig.height=10, fig.width=8, fig.align="center"}
...
(2) high variation in only PB showing coexistence of all groups at low ox diff and CB never collapsing on the decreasing ox diff trajectory, 



(3) high variation in both CB and PB. 
...
plot_here11 <- all_stab %>%
  filter(!(num_strains == 9 & var_treat == "CB" & stand_var > 1.00),
         ...
         !(num_strains == 9 & var_treat == "SB-PB" & stand_var > 1.0),
         !(num_strains == 9 & var_treat == "CB-SB-PB" & stand_var > 1.00),
         !(num_strains == -1 & var_treat == "CB" & stand_var > 1.00),
         !(num_strains == -1 & var_treat == "SB" & stand_var > 0.60),
         !(num_strains == -1 & var_treat == "PB" & stand_var > 1.00),
         !(num_strains == -1 & var_treat == "CB-SB" & stand_var > 0.60),
         !(num_strains == -1 & var_treat == "CB-PB" & stand_var > 1.00),
         !(num_strains == -1 & var_treat == "SB-PB" & stand_var > 1.00),
         !(num_strains == -1 & var_treat == "CB-SB-PB" & stand_var > 1.00))





plot_1 <- fig_div_vs_o2diff_maxtolcomp(plot_here11, which_strains,
                              which_wait_time, figure_title)
plot_1 +
...
When SB contained multiple strains varying in growth rate (and tolerance) there were only two stable states (Figure \@ref(fig:max-tol2)), as was previously reported. When, however, only one SB strain with very high tolerance and low maximum growth rate was present, there were three stable states: the two previously found, and another state at low oxygen diffusivity where all three functional groups coexisted (Figure \@ref(fig:max-tol3)) – this never occurred when there were multiple SB strains varying in tolerance.

Apparently, at low oxygen diffusivity levels, the absence of SB strains with high growth rate precludes the formation of a stable state that does not include cyanobacteria. The most tolerant SB strain has a low intrinsic growth rate, which leads to less production of sulfide. Presence of only the most tolerant SB strain therefore results in lower sulfide concentrations (Figure \@ref(fig:max-tol2) vs Figure \@ref(fig:max-tol3)), allowing growth of cyanobacteria at low oxygen diffusivity.
...
(ref:max-tol2) Stable states found when there is variation among nine strains of SB and the amount of variation is high. All other aspects are the same as in Figure \@ref(fig:repl-meth2). 
...
(ref:max-tol3) Stable states found when SB contained only the most tolerant strain, with trait values corresponding with the diversity level of the previous figure. All other aspects are the same as in Figure \@ref(fig:repl-meth2). 
...
In the SB-CB case, presence of only the most tolerant strain led to coexistence of all groups at low levels of oxygen diffusivity when trait values were moderately high (Figure xy) or to absence of alternate stable states and sole dominance of cyanobacteria when trait values were high (Figure \@ref(fig:max-tol5)). As discussed in the previous section, the low intrinsic growth rate of the most tolerant SB strain led to less production of sulfide such that SB could not exclude CB. 


Owen please add here a figure for medium trait values for the SB-CB case (only most tolerant strain), showing coexistence of all three groups at low ox diff. 

(ref:max-tol5)  Stable states found when CB and SB each contain only the single most tolerant strain with trait values corresponding with the diversity level of the previous figure. All other aspects are the same as in Figure \@ref(fig:repl-meth2). 
...
Whereas medium to high diversity in only PB led to three states including one with coexistence of all three functional groups (Figure xy), presence of only the most tolerant PB strain led to only two stable states, namely the standard-pattern of either dominance of sulfur bacteria or of cyanobacteria (Figure xy). The most tolerant PB strain has a low intrinsic growth rate, leading to less consumption of sulfide and therefore to higher sulfide concentrations compared to a system that contains both slow- and fast-growing strains. Due to the higher sulfide concentrations in the system with only the most tolerant PB strain, cyanobacteria were not able to grow at low oxygen diffusivity levels. 

For the same reasons we found three states on the trajectory of increasing oxygen diffusivity for medium to high simultaneous trait variation in PB and CB but not for the corresponding tolerance manipulation (data not shown).  

Owen please add here two figures:
(1) One for high trait variation in only PB showing coexistence of all three groups.
(2) One for high tolerance in only PB (corresponding in trait values to the figure before) showing standard dynamics.
...
At high levels of diversity vs. tolerance, resilience of the oxic state is greater when in each of the three functional groups only the most tolerant strain is present rather than when there is variation in strain tolerance within groups. It is not entirely clear why this effect occurs only in this combination. 
...
(ref:max-tol7) Stable states found with the single most tolerant CB strain, SB strain, and PB strain, with trait values corresponding with the diversity level in the previous figure. All other aspects are the same as in Figure \@ref(fig:repl-meth2). 
...
When two functional groups varied in traits, the effects of trait variation were additive when one of the groups had no diversity effect on a tipping point but non-additive when both groups had diversity effects on a tipping point (Fig. \@ref(fig:fig-add-linear) a-f). High trait variation in cyanobacteria and sulfate-reducing bacteria caused the oxic-anoxic tipping point to occur at lower oxygen diffusivity than expected assuming additivity of the individual effects of diversity in these two groups (Fig. \@ref(fig:fig-add-linear)d). When the two groups of sulfur bacteria varied in traits, the anoxic-oxic tipping point occurred at higher oxygen diffusivity than expected by additivity (Fig. \@ref(fig:fig-add-linear)f). 

When all three groups varied in traits, only the diversity effect in phototrophic sulfur bacteria on the oxic-anoxic tipping point was additive; all other effects were non-additive, especially at higher trait variation (Fig. \@ref(fig:fig-add-linear)g-i). 
