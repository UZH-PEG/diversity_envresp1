...
The code to run simulations, analyses, and visualisations is in [the `diversity_envresp1` repository](https://github.com/UZH-PEG/diversity_envresp1) from where it can be downloaded. It is also available on Zenodo [https://doi.org/10.5281/zenodo.6334543](https://doi.org/10.5281/zenodo.6334543) which points at the last released version. It includes:
...
zen4R::download_zenodo("10.5281/zenodo.7743866")
unzip("data.zip")
rm("microxanox_v1.0.0.zip")
```
...
download.file("https://zenodo.org/record/7743866/files/data.zip?download=1", "data.zip")
options(timeout = opt)

chksum <- tools::md5sum("./data.zip")
if (chksum != "66e0a5f53147fad4026ed1d6fabfd5a8") {
  stop(
...
Be aware that running simulations can take a long time (days). All data generated by the scripts and RMarkdown files is contained in the data deposit with the DOI [10.5281/zenodo.7743866](http://doi.org/10.5281/zenodo.7743866). See the section above about getting the data, if not done already.
...
To make the supplementary report: `reports/supplement/supplement_0.9.1.Rmd`: RMarkdown file used to generate [the supplementary report (HTML page)](https://uzh-peg.github.io/diversity_envresp1/reports/supplement/supplement_0.9.1.Rmd) to the publication.
