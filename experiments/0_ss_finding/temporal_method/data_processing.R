...
tbl1 <- list.files(path = here::here("data/0_ss_finding/temporal_method"),
                   pattern = "stab_data", 
                   full.names = TRUE,
                   recursive = FALSE) %>% 
  map_df(~read_plus(.))
...
all_stab <- tbl1 %>%
  select(-data, -pars, -parameter, -total_initial_abundances, -ssfind_pars, -ssfind_result) %>%
  separate(filename , sep = "_", into = c(paste0("junk", 1:7), "waittime", paste0("junk", 8:10))) %>%
  select(-starts_with("junk")) %>%
  mutate(waittime = parse_number(waittime))
...
saveRDS(all_stab_results, here::here("data/0_ss_finding/temporal_method/processed_data/stab_data_temporal_method.RDS"))
...
tbl1 <- list.files(path = here::here("data/0_ss_finding/temporal_method/event_def1"),
                   pattern = "stab_data", 
                   full.names = TRUE,
                   recursive = FALSE) %>% 
  map_df(~read_plus(.))
...
all_stab <- tbl1 %>%
  select(-data, -pars, -parameter, -total_initial_abundances, -ssfind_pars, -ssfind_result) %>%
  separate(filename , sep = "_", into = c(paste0("junk", 1:8), "waittime", paste0("junk", 9:11))) %>%
  select(-starts_with("junk")) %>%
  mutate(waittime = parse_number(waittime))
...
saveRDS(all_stab_results, here::here("data/0_ss_finding/temporal_method/event_def1/processed_data/stab_data_temporal_method.RDS"))
