...
tbl1 <- list.files(path = here("experiments/0_ss_finding/temporal_method/data"),
                   pattern = "stab_data", 
                   full.names = T,
                   recursive = T) %>% 
  map_df(~read_plus(.))
...
saveRDS(all_stab_results, here("experiments/0_ss_finding/temporal_method/processed_data/stab_data_temporal_method.RDS"))
