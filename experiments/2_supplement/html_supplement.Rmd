...
The results below show the stable states as a function of oxygen diffusivity for a system with intermediate level of diversity in all functional groups. There appears to be no effect of using a longer simulation time. This shows that the duration of the simulations is sufficient to reach stable state, or very close to it.
...
single_2e6 <- readRDS(here("experiments/0_ss_finding/temporal_method/data/single_1e6_result.RDS"))
# sort(unique(ss_9s$CB_var_gmax_s))
# sort(unique(ss_9s$SB_var_gmax_s))
# ss_result <- ss_9s %>%
#   filter(abs(CB_var_gmax_s - 0.028254848) < 0.0001,
