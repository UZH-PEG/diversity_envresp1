...
combine_fct <- function(Method="arithmetic"){
  agg_stab_res_single_groups_wide <- agg_stab_res %>%
    dplyr::filter(num_strains==9, Strain == "CB") %>%
    group_by(var_treat) %>%
...
agg_stab_strain9 %>%
  filter(Strain == "CB", num_strains==9) %>%
